...
- `release` — release branch. APK & AAB are built via `release.yml` on push to `release` or tag `v*`.
...
4. Ensure checks pass: `scripts/before_push.sh` (or `dart format`, `flutter analyze`, `flutter test --exclude-tags golden,screenshot`)
...
## Release
Merge `develop` → `release` or push tag `v*` to trigger APK & AAB build and GitHub Release.
