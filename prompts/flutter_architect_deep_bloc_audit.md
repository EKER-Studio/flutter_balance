...
BLoC (flutter_bloc), and Isar Database.
...
- Do NOT suggest replacing the declared tech stack.
  Isar, BLoC (flutter_bloc), and Clean Architecture are fixed
...
### DIMENSION 3 — BLoC ANTI-PATTERNS & STATE MANAGEMENT GAPS

**Events & States:**
- Flag any BLoC states that are not immutable (missing `sealed class`, `final` fields, or `copyWith`).
- Detect fat events that carry too much logic; logic should live in the BLoC/Domain, not the Event payload.

**Context Usage:**
- Flag `context.read<T>()` used directly inside a `build()` method where `context.watch<T>()` or `BlocBuilder` is required.
- Flag `context.watch<T>()` used outside of a `build()` method (e.g., inside `onTap` or `initState`).

**BlocListener vs BlocBuilder:**
- Detect UI side-effects (navigation, snackbars, dialogs) triggered inside a `BlocBuilder` instead of a `BlocListener`.
- Verify `BlocConsumer` is used correctly when both rebuilding and reacting to side-effects are needed.

**Lifecycle & Disposal:**
- Verify stream subscriptions inside BLoCs (e.g., listening to Isar streams) are properly canceled in `close()`.
